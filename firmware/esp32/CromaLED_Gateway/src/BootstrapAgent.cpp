...
#define AGENT_SERIAL_PRINTLN(...) do { if (serialLoggingEnabled_) Serial.println(__VA_ARGS__); } while (0)
#define AGENT_SERIAL_PRINTF(...) do { if (serialLoggingEnabled_) Serial.printf(__VA_ARGS__); } while (0)
...
bool BootstrapAgent::begin(const char *family,
  ...
  AGENT_SERIAL_PRINTF("[IDENTITY] device_id=%s, family=%s\n",
...
void BootstrapAgent::setSerialLoggingEnabled(bool enabled) {
  serialLoggingEnabled_ = enabled;
}
...
bool BootstrapAgent::handleFactoryResetPin() {
  ...
  if (digitalRead(IOT_FACTORY_RESET_PIN) == LOW) {
    AGENT_SERIAL_PRINTLN("[FACTORY] Reset pin active. Hold for 3 seconds...");
    ...
    if (millis() - start >= 3000UL) {
      ...
      if (!storage_.clearAll(error)) {
        ...
        AGENT_SERIAL_PRINTLN("[FACTORY] Identity and credentials cleared");
      }
      return true;
    }
  }
...
bool BootstrapAgent::enterFactoryIdentityMode() {
  ...
  while (true) {
    ...
    if (jsonError) {
      AGENT_SERIAL_PRINTF("FACTORY_ERROR JSON:%s\n", jsonError.c_str());
      continue;
    }
    ...
    if (command != "set_identity" || identity.deviceId != expectedDeviceId ||
      ...
      AGENT_SERIAL_PRINTLN("FACTORY_ERROR INVALID_IDENTITY");
      continue;
    }

    String error;
    if (!storage_.storeBootstrapIdentity(identity, error)) {
      AGENT_SERIAL_PRINTF("FACTORY_ERROR STORE:%s\n", error.c_str());
      continue;
    }

    AGENT_SERIAL_PRINTF("FACTORY_OK %s\n", identity.deviceId.c_str());
    Serial.flush();
    delay(750);
    ESP.restart();
  }
...
bool BootstrapAgent::connectWifi() {
  ...
  if (String(IOT_WIFI_SSID) == "CHANGE_SSID" || String(IOT_WIFI_SSID).isEmpty()) {
    AGENT_SERIAL_PRINTLN("[WIFI] Configure AgentConfig.h before building");
    return false;
  }
  ...
  AGENT_SERIAL_PRINTF("[WIFI] Connecting to %s", IOT_WIFI_SSID);
  ...
  if (WiFi.status() != WL_CONNECTED) {
    AGENT_SERIAL_PRINTF("[WIFI] Connection failed, state=%d\n", static_cast<int>(WiFi.status()));
    return false;
  }
  AGENT_SERIAL_PRINTF("[WIFI] Local IP: %s, RSSI: %d dBm\n",
...
bool BootstrapAgent::synchronizeClock() {
  ...
  if (String(IOT_TIME_SIGNING_PUBLIC_KEY).indexOf("BEGIN PUBLIC KEY") < 0) {
    AGENT_SERIAL_PRINTLN("[TIME] Signed local-time public key is not configured");
    return false;
  }
  ...
  AGENT_SERIAL_PRINTF("[TIME] Requesting signed local time from %s:%u\n",
  ...
  if (!http.begin(plainClient, url)) {
    AGENT_SERIAL_PRINTLN("[TIME] Could not initialize local-time HTTP request");
    return false;
  }
  ...
  http.end();
  if (statusCode != 200) {
    AGENT_SERIAL_PRINTF("[TIME] Local-time service failed, HTTP=%d\n", statusCode);
    return false;
  }
  ...
  if (jsonError) {
    AGENT_SERIAL_PRINTF("[TIME] Invalid local-time JSON: %s\n", jsonError.c_str());
    return false;
  }
  ...
  if (protocol != "IOT-SIGNED-TIME-V1" || returnedNonce != nonce ||
    ...
    AGENT_SERIAL_PRINTLN("[TIME] Signed local-time response failed structural validation");
    return false;
  }
  ...
  if (settimeofday(&tv, nullptr) != 0 || time(nullptr) <= 1700000000) {
    AGENT_SERIAL_PRINTLN("[TIME] Could not set ESP32 system clock");
    return false;
  }

  AGENT_SERIAL_PRINTF("[TIME] Signed local time accepted. UTC=%s\n", utcTimestamp().c_str());
  return true;
}
...
bool BootstrapAgent::ensureProvisioned() {
  ...
  if (storage_.hasOperationalCredentials()) {
    ...
    provisioned_ = true;
    AGENT_SERIAL_PRINTF("[BOOTSTRAP] Persistent credentials found. Serial=%s\n",
...
bool BootstrapAgent::requestOperationalCredentials() {
  ...
  AGENT_SERIAL_PRINTLN("[BOOTSTRAP] No operational credentials: starting provisioning");

  GeneratedIdentity generated;
  String error;
  AGENT_SERIAL_PRINTLN("[BOOTSTRAP] Generating EC P-256 key and CSR locally");
  ...
  if (statusCode != HTTP_CODE_CREATED) {
    AGENT_SERIAL_PRINTF("[BOOTSTRAP] Challenge rejected HTTP %d: %s\n", statusCode,
                  responseBody.c_str());
    return false;
  }

  AGENT_SERIAL_PRINTF("[METRIC] challenge_http_ms=%lu\n", millis() - challengeStartMs);
  ...
  if (statusCode != HTTP_CODE_OK) {
    AGENT_SERIAL_PRINTF("[BOOTSTRAP] Enrollment rejected HTTP %d: %s\n", statusCode,
                  responseBody.c_str());
    return false;
  }

  AGENT_SERIAL_PRINTF("[METRIC] enroll_http_ms=%lu\n", millis() - enrollmentHttpStartMs);
  ...
  provisioned_ = true;
  AGENT_SERIAL_PRINTF("[METRIC] provisioning_total_ms=%lu\n", millis() - provisioningStartMs);
  AGENT_SERIAL_PRINTF("[BOOTSTRAP] Provisioning completed. Serial=%s, MQTT=%s:%u\n",
...
bool BootstrapAgent::connectMqtt() {
  ...
  AGENT_SERIAL_PRINTF("[MQTT] Connected. Subscribed to %s\n",
...
void BootstrapAgent::onMqttMessage(char *topic, byte *payload, unsigned int length) {
  ...
  if (length == 0 || length > 4095) {
    AGENT_SERIAL_PRINTLN("[MQTT] Empty or oversized command");
    return;
  }
  ...
  if (jsonError) {
    AGENT_SERIAL_PRINTF("[MQTT] Invalid command JSON: %s\n", jsonError.c_str());
    return;
  }
